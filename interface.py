...
def check_input(args):
    ...
    characters"""
    actions = ['list', 'empty', 'size', 'add', 'remove', 'description', 'set_description', 'find']
...
if go_on:
    if args[0] == 'new':
        try: # check if the tree the user wants to create already exist in the directory
            with open(args[1], "rb") as my_file:
                print "The trie you wanted to create already exists"
        except:
            my_lex = Trie(args[0])
            print "'", args[1], "' trie created"
        ...
        try:# check whether the tree the user wants to access already exist or not
            ...
            my_lex = Trie(args[0])
            print "'", args[0], "' trie didn't exist so it's been created"
